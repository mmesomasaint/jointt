...
export default function Host() {
  ...
  return (
    <div className='p-3'>
      ...
      {activeStage === 1 && <ChooseEventType setEventName={setEventName} setEventType={setEventType} validated={!(eventName && eventType)} />}
      {activeStage === 2 && <ChooseContractorTypes />}
    </div>
  )
}

function ChooseEventType ({setEventName, setEventType, validated}: {setEventName: (name: string) => void, setEventType: (type: string) => void, validated: boolean}) {
  return (
  <div className='flex flex-col gap-4 justify-start items-start w-fit mx-auto mt-16'>
    ...
    <Button type='button' disabled={validated} className='disabled:bg-gray-600/50 disabled:border-gray-600/50'>
      Next
    </Button>
  </div>
)
